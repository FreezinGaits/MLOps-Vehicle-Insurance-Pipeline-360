...
class SimpleStorageService:
    ...
    def load_model(self, model_name: str, bucket_name: str, model_dir: str = None) -> object:
        ...
        """
        try:
            model_file = model_dir + "/" + model_name if model_dir else model_name # Builds full S3 key
            file_object = self.get_file_object(model_file, bucket_name) # Fetches S3 object
            model_obj = self.read_object(file_object, decode=False) # Reads raw bytes (important!)
            model = pickle.loads(model_obj) # Deserializes model into memory
